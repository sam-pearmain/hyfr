...
from pyfr.solvers.baseadvec import (
...
class FluidIntIntersMixin:
    def __init__(self, *args, **kwargs):
        ...
        if self._ef_enabled:
            self._be.pointwise.register("pyfr.solvers.eqeuler.kernels.intcent")
...
class TplargsMixin:
    def __init__(self, *args, **kwargs):
        ...
        if self.cfg.get("solver", "shock-capturing") == "entropy-filter":
            self.p_min = self.cfg.getfloat("solver-entropy-filter", "p-min", 1e-6)
        else:
            self.p_min = self.cfg.getfloat(
                "solver-interfaces", "p-min", 5 * self._be.fpdtype_eps
            )
...
class EquilibriumEulerIntInters(
    TplargsMixin, FluidIntIntersMixin, BaseAdvectionIntInters
):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
...
class EquilibriumEulerMPIInters(
    TplargsMixin, FluidMPIIntersMixin, BaseAdvectionMPIInters
):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
...
class EquilibriumEulerBaseBCInters(TplargsMixin, BaseAdvectionBCInters):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
...
class EquilibriumEulerSupInflowBCInters(EquilibriumEulerBaseBCInters):
    type = "sup-in-fa"

    def __init__(self, be, lhs, elemap, cfgsect, cfg, bccomm):
        super().__init__(be, lhs, elemap, cfgsect, cfg, bccomm)

        self.c |= self._exp_opts(["rho", "p", "u", "v", "w"][: self.ndims + 2], lhs)


class EquilibriumEulerSupOutflowBCInters(EquilibriumEulerBaseBCInters):
    type = "sup-out-fn"
    cflux_state = "ghost"


class EquilibriumEulerCharRiemInvBCInters(EquilibriumEulerBaseBCInters):
    type = "char-riem-inv"
...
class EquilibriumEulerSlpAdiaWallBCInters(EquilibriumEulerBaseBCInters):
    type = "slp-adia-wall"
...
class MassFlowBCMixin:
    def __init__(self, be, lhs, elemap, cfgsect, cfg, bccomm):
        ...
        self.c |= self._exp_opts(["rho", "u", "v", "w"][: self.ndims + 1], lhs)
...
class EquilibriumEulerCharRiemInvMassFlowBCInters(
    MassFlowBCMixin, EquilibriumEulerBaseBCInters
):
