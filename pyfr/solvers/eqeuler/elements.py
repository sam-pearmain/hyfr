...
class BaseFluidElements:
    ...
    def visvars(ndims, cfg):
        if ndims == 2:
            return {"density": ["rho"], "velocity": ["u", "v"], "pressure": ["p"]}
        elif ndims == 3:
            return {"density": ["rho"], "velocity": ["u", "v", "w"], "pressure": ["p"]}
    ...
    def diff_con_to_pri(cons, diff_cons, cfg):
        ...
        diff_uvw = [
            (diff_rhov - v * diff_rho) / rho for diff_rhov, v in zip(diff_rhouvw, uvw)
        ]

        # Pressure gradient: ∂p = (γ - 1)·[∂E - 1/2*(u⃗·∂(ρu⃗) + ρu⃗·∂u⃗)]
        gamma = cfg.getfloat("constants", "gamma")
        diff_p = diff_E - 0.5 * (
            sum(u * dru for u, dru in zip(uvw, diff_rhouvw))
    ...
    def validate_formulation(ctrl):
        ...
        if shock_capturing == "entropy-filter":
            if ctrl.formulation == "dual":
                raise ValueError(
                    "Entropy filtering not compatible with dual time stepping."
                )
            elif ctrl.controller_has_variable_dt:
                raise ValueError(
                    "Entropy filtering not compatible with adaptive time stepping."
                )
    ...
    def set_backend(self, *args, **kwargs):
        ...
        if shock_capturing == "entropy-filter" and self.basis.order != 0:
            ...
            if self.antialias:
                raise ValueError("Entropy filter not compatible with anti-aliasing.")

            # Minimum density/pressure constraints
            eftplargs["d_min"] = self.cfg.getfloat(
                "solver-entropy-filter", "d-min", 1e-6
            )
            eftplargs["p_min"] = self.cfg.getfloat(
                "solver-entropy-filter", "p-min", 1e-6
            )

            # Entropy tolerance
            eftplargs["e_tol"] = self.cfg.getfloat(
                "solver-entropy-filter", "e-tol", 1e-6
            )

            # Inner solver parameters
            eftplargs["f_tol"] = self.cfg.getfloat(
                "solver-entropy-filter", "f-tol", 1e-4
            )
            eftplargs["niters"] = self.cfg.getfloat(
                "solver-entropy-filter", "niters", 2
            )

            # Use linearised constraints/limiting kernel approach from
            # Ching et al. (doi:10.1016/j.jcp.2024.112881)
            form = self.cfg.get("solver-entropy-filter", "formulation", "nonlinear")
...
class RealGasFluidElements(BaseFluidElements):
    pass
...
class EulerElements(BaseFluidElements, BaseAdvectionElements):
    def set_backend(self, *args, **kwargs):
        ...
        if c in r and "flux" not in self.antialias:
            tdisf.append(
                lambda uin: self._be.kernel(
                    "tflux",
                    tplargs=tplargs | {"ktype": "curved"},
                    dims=[self.nupts, r[c]],
                    u=s(self.scal_upts[uin], c),
            ...
            tdisf.append(
                lambda: self._be.kernel(
                    "tflux",
                    tplargs=tplargs | {"ktype": "curved"},
                    dims=[self.nqpts, r[c]],
                    u=s(self._scal_qpts, c),
        ...
        if l in r and "flux" not in self.antialias:
            tdisf.append(
                lambda uin: self._be.kernel(
                    "tflux",
                    tplargs=tplargs | {"ktype": "linear"},
                    dims=[self.nupts, r[l]],
                    u=s(self.scal_upts[uin], l),
