...
from pyfr.partitioners import (
    BasePartitioner,
    get_partitioner,
    reconstruct_partitioning,
    write_partitioning,
)
from pyfr.plugins import BaseCLIPlugin
from pyfr.progress import NullProgressSequence, ProgressBar, ProgressSequenceAction
from pyfr.readers import BaseReader, get_reader_by_extn, get_reader_by_name
from pyfr.readers.native import NativeReader
from pyfr.readers.stl import read_stl
from pyfr.resamplers import BaseInterpolator, NativeCloudResampler, get_interpolator
...
def main():
    ...
    # Common options
    ap.add_argument("-v", "--verbose", action="count", help="increase verbosity")
    ap.add_argument(
        "-V", "--version", action="version", version=f"%(prog)s {__version__}"
    )
    ap.add_argument(
        "-p", "--progress", action=ProgressSequenceAction, help="show progress"
    )
    ...
    ap_import.add_argument(
        "-l", dest="lintol", type=float, default=1e-5, help="linearisation tolerance"
    )
    ...
    ap_partition_add.add_argument(
        "-p", dest="partitioner", choices=partitioners, help="partitioner to use"
    )
    ap_partition_add.add_argument(
        "-e",
        dest="elewts",
    ...
    ap_partition_reconstruct.set_defaults(process=process_partition_reconstruct)
    ...
    for etype in ("boundary", "stl", "volume"):
        ap_export_type = ap_export.add_parser(etype, help=f"export {etype} --help")
        ...
        if etype == "boundary":
            ap_export_type.add_argument(
                "eargs", nargs="+", metavar="boundary", help="boundary to output"
            )
        elif etype == "stl":
            ap_export_type.add_argument(
                "eargs", nargs="+", metavar="stl", help="STL region to output"
            )
        ...
        ap_export_type.add_argument(
            ...
            default="single",
            help="output number precision; defaults to single",
        )
        ap_export_type.add_argument(
            "--eopt",
            dest="eopts",
            ...
            default=[],
            metavar="key:value",
            help="exporter-specific option",
        )
        ap_export_type.add_argument("-P", "--pname", help="partitioning to use")
    ...
    # Remove region
    ap_region_remove = ap_region.add_parser("remove", help="region remove --help")
    ...
    ap_resample.add_argument(
        ...
        default=[],
        metavar="key:value",
        help="interpolator-specific option",
    )
    ...
    ap_restart.add_argument(
        "cfg", nargs="?", type=FileType("r"), help="new config file"
    )
    ...
    for p in [ap_run, ap_restart]:
        p.add_argument(
            "-b", "--backend", choices=backends, required=True, help="backend to use"
        )
...
def process_partition_reconstruct(args):
    with h5py.File(args.mesh, "r+") as mesh, h5py.File(args.soln, "r") as soln:
...
def process_region_list(args):
    with h5py.File(args.mesh, "r") as mesh:
        ...
        for name, tris in sorted(mesh.get("regions/stl", {}).items()):
            print(name, len(tris), str(tris.attrs["closed"]).lower(), sep=args.sep)
...
def process_export(args):
    ...
    kwargs = {"fields": args.fields, "prec": args.precision, "pname": args.pname}
    ...
    if args.ftype:
        writer = get_writer_by_name(
            args.ftype, args.etype, args.meshf, *kargs, **kwargs
        )
    else:
        extn = Path(batch[0][1]).suffix
        writer = get_writer_by_extn(extn, args.etype, args.meshf, *kargs, **kwargs)
...
def process_resample(args):
    ...
    with progress.start("Write target solution"):
        ...
        writer = NativeWriter(
            treader.mesh, tcfg, fpdtype, tpath.parent, tpath.name, prefix
        )
